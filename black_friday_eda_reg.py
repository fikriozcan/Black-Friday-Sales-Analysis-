...
# Distribution of each variable in data
...
for label, a, b in zip(label_low, labels_min['Product_Category_1'], labels_min['Purchase']):
    ax[1,0].annotate(
        ...
        bbox=dict(boxstyle='round,pad=0.5', fc='gold', alpha=0.5),
        arrowprops=dict(arrowstyle = '->', connectionstyle='arc3,rad=0'))
ax[1,0].set_title('Price & Sales Relationship per Product category 1')
...
for label, a, b in zip(label_low_2, labels_min_2['Product_Category_2'], labels_min_2['Purchase']):
    ax[0,0].annotate(
        ...
        bbox=dict(boxstyle='round,pad=0.5', fc='gold', alpha=0.5),
        arrowprops=dict(arrowstyle = '->', connectionstyle='arc3,rad=0'))
ax[0,0].set_title('Price & Sales Relationship per product category2')
...
for label, a, b in zip(label_low_3, labels_min_3['Product_Category_3'], labels_min_3['Purchase']):
    ax[0,1].annotate(
        ...
        bbox=dict(boxstyle='round,pad=0.5', fc='gold', alpha=0.5),
        arrowprops=dict(arrowstyle = '->', connectionstyle='arc3,rad=0'))
ax[0,1].set_title('Price & Sales Relationship per product category3')
...
bf_with_dummies.loc[:, "combined_G_M"] = pd.factorize(bf_with_dummies.combined_G_M)[0]# suna bak 
